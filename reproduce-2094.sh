...
docker run -e REGISTRY_STORAGE_DELETE_ENABLED=true -d --net=dev -p 5000:5000 --name registry registry

echo "===== Install tree in registry container ====="
docker exec -ti registry apk add --no-cache tree

docker exec -ti registry tree /var/lib/registry
...
docker exec -ti registry tree /var/lib/registry
docker exec -ti registry du -sh /var/lib/registry
